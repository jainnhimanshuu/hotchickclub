...
class HotChickBuilder {
  ...
  async build() {
    ...
    try {
      // Try to clean dist directory, but don't fail if it has permission issues
      try {
        await fs.remove(this.distDir);
      } catch (error) {
        console.log(
          "⚠️  Could not clean dist directory (permission issue), continuing..."
        );
      }
  ...
  async copyAssets() {
    ...
    // Copy assets directory with error handling
    if (await fs.pathExists("assets")) {
      try {
        ...
        console.log(
          "⚠️  Some assets could not be copied (permission issue), continuing..."
        );
      }
    }
    ...
    for (const file of otherFiles) {
      if (await fs.pathExists(file)) {
        try {
          ...
          console.log(`⚠️  Could not copy ${file}, continuing...`);
        }
      }
    }
  }
